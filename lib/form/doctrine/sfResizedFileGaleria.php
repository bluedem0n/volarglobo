 ...
 * @version    0.1
 */
class sfResizedFileGaleria extends sfValidatedFile
{
	...
	public function save($file = null, $fileMode = 0666, $create = true, $dirMode = 0777)
	{
		...
		if ($file[0] != '/' && $file[0] != '\\' && !(strlen($file) > 3 && ctype_alpha($file[0]) && $file[1] == ':' && ($file[2] == '\\' || $file[2] == '/')))
		{
			if (is_null($this->path))
			{
				throw new RuntimeException('You must give a "path" when you give a relative file name.');
                        ...
                        $fileG = $this->path.DIRECTORY_SEPARATOR.'g_'.$file;
			$medioFile = $this->path.DIRECTORY_SEPARATOR.'m_'.$file;                        
		...
		$thumbnail = new sfThumbnail(120, 120, true, true, 100, '');
		$thumbnail->loadFile($this->getTempName());
		$thumbnail->save($smallFile, 'image/jpeg');
                
                $thumbnail = new sfThumbnail(200, 200, true, true, 100, '');
		$thumbnail->loadFile($this->getTempName());
		$thumbnail->save($medioFile, 'image/jpeg');
                
                $thumbnail = new sfThumbnail(800, 800, true, true, 100, '');
		$thumbnail->loadFile($this->getTempName());
		$thumbnail->save($fileG, 'image/jpeg');

		$thumbnail = new sfThumbnail(400, 400, true, true, 100, '');
		$thumbnail->loadFile($this->getTempName());
