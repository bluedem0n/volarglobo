        ...
        <?php }?>
          ...
          <div class="row">
            ...
            <div class="col-md-12">
                ...
                <!-- form start -->
                <form class="filtroRangos" role="form" id="myForm" action="<?php echo url_for('/hospedaje/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
                ...
                <ul class="nav nav-tabs">
                  ...
                  <li><a href="#tab_3" data-toggle="tab">Habitaciones</a></li>
...
<!--                  <li><a href="#tab_8" data-toggle="tab">Catalogo</a></li>-->
                ...
                <div class="tab-content">
                  ...
                  <div class="box-body">
                    ...
                    <div class="form-group col-md-4 <?php if($form['nombre']->renderError()) echo "has-error"; ?>">
                      <label for="nickname"><?php if($form['nombre']->renderError()) echo '<i class="fa fa-times-circle-o"></i>'; ?>Nombre hospedaje</label>
                ...
                <div class="box-header">
                  <h3 class="box-title">Habitaciones del hospedaje</h3>
                  <a  style="margin-left: 10px;" href="javascript:null(0)" id='add_habitaciones'>Agregar Nueva</a>
                </div><!-- /.box-header -->
                  ...
                  <table class="table table-striped">
                    <tbody id='extrahabitaciones'>
                     <tr >
                      ...
                      <th>Cant.P</th>
                      <th>Opciones de cama</th>
                      <th>precio</th>
                      <th>foto</th>
                      <th></th>
                    </tr>
                    
                    <?php for($i=0;$i<count($form['habitaciones']);$i++){ ?>
                    <tr  id="habitaciones<?php echo $i ?>">
                      
                      <td><?php echo $form['habitaciones'][$i]['nombre'] ?></td>
                      <td><?php echo $form['habitaciones'][$i]['tipo_habitacion_id'] ?></td>
                      <td><?php echo $form['habitaciones'][$i]['cantidad_personas'] ?></td>
                      <td style="font-size: 10px"><?php echo $form['habitaciones'][$i]['tipo_cama'] ?></td>
                      <td><?php echo $form['habitaciones'][$i]['precio'] ?></td>
                      
                      
                    <td>
                         
                         <button class="btn btn-social-icon btn-google-plus" onclick="removeEtiqueta('habitaciones<?php echo $i ?>','<?php echo url_for('hospedaje/delHabitacion')."?id=".$form['habitaciones'][$i]['id']->getValue() ?>')" type="button"><i class="fa fa-bitbucket"></i></button>
...
<script type="text/javascript">
    ...
    $('#hospedaje_ciudad_id').addClass('form-control');
    $('#hospedaje_provincia_id').addClass('form-control');
    
    
    
var prec = <?php print_r($form['habitaciones']->count())?>;


 $('#add_habitaciones').click(function() {
    
    $("#extrahabitaciones").append(addHabitacion(prec));
    prec = prec + 1;
  });


function addHabitacion(num) {
  var r = $.ajax({
    type: 'GET',
    url: '<?php echo url_for('hospedaje/addHabitacionForm')?>'+'<?php echo   ($form->getObject()->isNew()?'':'?id='.$form->getObject()->getId()).($form->getObject()->isNew()?'?num=':'&num=')?>'+num,
    async: false
  }).responseText;
  return r;
}
...
function addGaleria(num) {
  ...
  var r = $.ajax({
    type: 'GET',
    url: '<?php echo url_for('/hospedaje/addGaleriaForm')?>'+'<?php echo   ($form->getObject()->isNew()?'':'?id='.$form->getObject()->getId()).($form->getObject()->isNew()?'?num=':'&num=')?>'+num,
    async: false
  }).responseText;
  return r;
}
...
function addRed(num) {
  var r = $.ajax({
    type: 'GET',
    url: '<?php echo url_for('hospedaje/addRedForm')?>'+'<?php echo   ($form->getObject()->isNew()?'':'?id='.$form->getObject()->getId()).($form->getObject()->isNew()?'?num=':'&num=')?>'+num,
    async: false
  }).responseText;
  return r;
}
